...
def _surprise_zone(zscore: float) -> tuple[str, str]:
    magnitude = abs(float(zscore))
    if magnitude < 1:
        return "#22c55e", "Volume expected within normal range — no pre-scaling needed"
    if magnitude < 2:
        return "#f59e0b", "Mild volume deviation — increased monitoring recommended"
    if magnitude < 3:
        return "#f59e0b", "Significant volume deviation — consider pre-scaling infrastructure"
    return "#ef4444", "Extreme volume deviation — pre-scale and activate incident readiness"


def _meter_angle(zscore: float) -> float:
    value = float(zscore)
    angle = 270 + (value * 23.33 if value <= 0 else value * 11.67)
    return max(200.0, min(340.0, angle))
...
def build_deviation_meter(
    ...
    tick_positions = {-3: 200, -1: 246.67, 0: 270, 1: 281.67, 3: 305, 6: 340}
    ticks = "".join(
        f"<text x='{_polar_point(angle_value, 134)[0]:.1f}' y='{_polar_point(angle_value, 134)[1]:.1f}' "
        f"text-anchor='middle' dominant-baseline='middle' style='font-size:10px;font-family:monospace;fill:#737373'>{label:+d}σ</text>"
        for label, angle_value in tick_positions.items()
    )
    zones = [
        (200, 223.33, "rgba(239,68,68,0.35)"),
        (223.33, 246.67, "rgba(245,158,11,0.35)"),
        (246.67, 281.67, "rgba(34,197,94,0.35)"),
        (281.67, 305.0, "rgba(245,158,11,0.35)"),
        (305.0, 340.0, "rgba(239,68,68,0.35)"),
    ]
    zone_paths = "".join(
    ...
    <div style='background:#111111;border:.5px solid rgba(255,255,255,.08);border-radius:12px;padding:24px 28px;box-shadow:0 1px 3px rgba(0,0,0,.4);margin:16px 0'>
      ...
      <div style='display:flex;justify-content:center'>
        <svg viewBox='0 0 320 180' width='100%' style='max-width:520px;height:auto'>
          <path d='{_arc_path(200, 340)}' fill='none' stroke='#2a2a2a' stroke-width='18' stroke-linecap='round'/>
          {zone_paths}
          {ticks}
          ...
          <text x='160' y='130' text-anchor='middle' style='font-size:22px;font-weight:500;font-family:monospace;fill:{zone_color}'>{zscore:+.2f}σ</text>
          <text x='160' y='146' text-anchor='middle' style='font-size:10px;fill:#737373'>forecast surprise</text>
        </svg>
      </div>
      {metrics}
