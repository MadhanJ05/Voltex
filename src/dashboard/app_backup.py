...
def live_shap_values() -> list[tuple[str, float]]:
    ...
    try:
        ...
        return dashboard_classifier().explain_prediction(ticker_row)
...
def render_replay(row: dict, mode: str = "replay") -> None:
    ...
    with left:
        ...
        if not chart.empty and mode == "replay": render_replay_evidence_chart(chart, tier, float(row.get("forecast_surprise_zscore", 0.0)))
        elif not chart.empty: render_volume_chart(chart)
    ...
    with right:
        ...
        shap_values = replay_shaps if mode == "replay" else live_shap_values()
        if not shap_values:
